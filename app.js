...
const defaultState = {
  ...
  settings: {
    ...
    privacyCurtain: true, lastBackupAt: null
  },
...
function backupReminder(){if(!state.settings.lastBackupAt)return `<section class="card"><p class="eyebrow">Protect your history</p><h3>Create your first backup</h3><p class="muted">Browser data can be removed if the app or website data is deleted.</p><button class="secondary-button" data-action="export-backup">Export backup</button></section>`;const age=daysBetween(state.settings.lastBackupAt.slice(0,10),todayISO());return age>=30?`<section class="card"><h3>Your last backup was ${age} days ago</h3><button class="secondary-button" data-action="export-backup">Create a fresh backup</button></section>`:''}
...
async function init(){
  try{await openDB();const saved=await dbGet();if(saved)state={...structuredClone(defaultState),...saved,settings:{...defaultState.settings,...saved.settings}};else await dbPut(state)}catch(e){
    storageMode='localstorage';
    try{const saved=localStorage.getItem(DB_NAME);if(saved)state={...structuredClone(defaultState),...JSON.parse(saved),settings:{...defaultState.settings,...JSON.parse(saved).settings}};else localStorage.setItem(DB_NAME,JSON.stringify(state));}
...
if (globalThis.__LUNA_TEST_MODE__) {
  globalThis.__LUNA_TEST_API__ = {
    parseISO, dateToISO, addDays, daysBetween, clamp, mean, rebuildPeriods, cycleData,
